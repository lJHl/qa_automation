...
from framework.urls.urls import Urls
...
class Browser(metaclass=Singleton):
    ...
    def set_up_driver(self, selenoid, browser_key, lang, capabilities=None, is_incognito=False,
        ...
        if browser_key in self.__web_driver:
            raise ValueError("Browser with key '{}', was created.".format(browser_key))
    ...
    def select_browser(self, browser_key=BrowserConfig.BROWSER):
        if browser_key not in self.__web_driver:
            raise KeyError("Browser with key '{}', is not exist.".format(browser_key))
    ...
    def get_default_url(self):
        self.get_driver().get(Urls.TEST_STAND_URL)
    ...
    def get_count_windows(self):
        ...
        Logger.info("Count open windows: %d" % count)
        return count
    ...
    def switch_to_frame_by_name(self, frame_name):
        Logger.info("Switch to frame with name %s" % frame_name)
        self.get_driver().switch_to_frame(frame_name)

    def switch_to_frame_by_locator(self, search_condition, locator):
        Logger.info("Switch to frame with locator {}".format(locator))
    ...
    def get_alert_text(self):
        Logger.info("Return text from alert")
